...
export default function TradeFinder({ userSelectedPicks, userValue, allPicks, userTeam, onSelectTrade, getPickValue }: TradeFinderProps) {
  ...
  const offers = useMemo(() => {
    ...
    // Shuffle teams to ensure variety and help reach the 3-7 requirement
    ...
    teams.forEach(team => {
      ...
      if (isHighValueTrade) {
        for (let i = 0; i < Math.min(teamPicks.length, 6); i++) {
          for (let j = i + 1; j < Math.min(teamPicks.length, 7); j++) {
            ...
            // Enforce "Overpay" logic (ratio can go up to 1.30 for high picks)
      ...
      // --- LOGIC B: VALUE PACKAGE (No 1-for-1s) ---
      if (!isHighValueTrade) {
        teamPicks.forEach(p => {
            const pVal = getPickValue(p);
            const sweetener = combinedPool.find(s => s.id !== p.id && getPickValue(s) < 200);
            
            if (sweetener && p.slot_number !== highestUserPick.slot_number) {
                const combo = [p, sweetener];
                const totalVal = combo.reduce((sum, cp) => sum + getPickValue(cp), 0);
                const ratio = totalVal / userValue;
                
                if (ratio >= 0.85 && ratio <= 1.25) {
                    results.push({ team, picks: combo, totalValue: totalVal, label: "Value Package" });
                }
            }
        });
      }
    ...
    // Unique teams only
    let finalOffers = results
      .sort((a, b) => Math.abs(1 - (a.totalValue / userValue)) - Math.abs(1 - (b.totalValue / userValue)))
      .filter((v, i, a) => a.findIndex(t => t.team === v.team) === i);

    // --- GUARANTEE 3-7 OFFERS ---
    const targetCount = Math.floor(Math.random() * 5) + 3; // Randomly 3, 4, 5, 6, or 7

    if (finalOffers.length < targetCount) {
      const remainingTeams = teams.filter(t => !finalOffers.some(o => o.team === t));
      remainingTeams.slice(0, targetCount - finalOffers.length).forEach(team => {
        const picks = allPicks.filter(p => p.current_team_name === team).slice(0, 2);
        // Ensure even fallbacks are at least 2 picks (No 1-for-1s)
        if (picks.length >= 2) {
          finalOffers.push({
            team,
            picks,
            totalValue: picks.reduce((s, p) => s + getPickValue(p), 0),
            label: "Inbound Offer"
          });
        }
      });
    ...
    return finalOffers.slice(0, targetCount);
        ...
        <div className="mt-3 space-y-2 max-h-64 overflow-y-auto pr-1 custom-scrollbar">
          {offers.length === 0 ? (
            <div className="p-4 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
              <p className="text-slate-400 text-[10px] font-bold uppercase italic tracking-tight text-balance">Searching for viable partners...</p>
            </div>
          ) : (
            offers.map((offer, idx) => (
              ...
              >
                ...
                <div className="text-right">
                  <div className="flex flex-wrap gap-1 justify-end mb-1 max-w-[140px]">
                    {offer.picks.map((p, i) => (
                      <span key={i} className={`px-1.5 py-0.5 text-white text-[7px] font-black rounded uppercase ${(p.year || 0) > 2026 ? 'bg-amber-500 shadow-sm shadow-amber-200' : 'bg-slate-900'}`}>
                        {(p.year || 0) > 2026 ? `'${p.year?.toString().slice(-2)} R${p.round}` : `R${p.round} #${p.slot_number}`}
                      </span>
                    ))}
                  </div>
